...
def _get_model(config: dict):
    ...
    if model_type == "catboost" and CATBOOST_AVAILABLE:
        ...
        raise ImportError(
            "No ML library available. Please install catboost, lightgbm, or scikit-learn"
        )
...
def train_model(features_df: pd.DataFrame, config: dict) -> tuple:
    ...
    if features_df.empty:
        ...
        if "quantity" in features_df.columns:
            x_features = features_df[available_cols].fillna(0)
            y = features_df["quantity"]
            model.fit(x_features, y)
            logger.info(f"Trained on {len(x_features)} samples with {len(available_cols)} features")
        else:
    ...
    # Create metrics dict
    metrics = {"mae": 0.0, "wape": 0.0, "model_type": config["model"]["type"]}
