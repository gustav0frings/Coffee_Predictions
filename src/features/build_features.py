...
def build_features(sales_df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    if sales_df.empty:
        ...
        return pd.DataFrame(columns=[
            ...
            "promotion_discount", "is_holiday", "quantity"
        ])
    ...
    if "promotion_discount" not in feature_df.columns:
        ...
        feature_df["promotion_discount"] = feature_df["promotion_discount"].fillna(0.0).astype(float)
    ...
    feature_df["rolling_7"] = feature_df.groupby("item_id")["quantity"].transform(
        lambda x: x.rolling(window=7, min_periods=1).mean()
    ).fillna(0)
    feature_df["rolling_28"] = feature_df.groupby("item_id")["quantity"].transform(
